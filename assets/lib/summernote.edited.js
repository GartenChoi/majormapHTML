...
(function (global, factory) {
  ...
  (global = global || self, factory(global.jqNoConflict));
  ...
  var TooltipUI = /** @class */ (function () {
      function TooltipUI($node, options) {
          this.$node = $node;
          this.options = jqNoConflict.extend({}, {
              title: '',
  ...
  var DropdownUI = /** @class */ (function () {
      ...
      DropdownUI.prototype.clear = function () {
          var $parent = jqNoConflict('.note-btn-group.open');
      ...
      DropdownUI.prototype.show = function () {
          ...
          var windowWidth = jqNoConflict(window).width();
          var targetMarginRight = parseFloat(jqNoConflict(this.options.target).css('margin-right'));
  ...
  jqNoConflict(document).on('click', function (e) {
      if (!jqNoConflict(e.target).closest('.note-btn-group').length) {
          jqNoConflict('.note-btn-group.open').removeClass('open');
      }
  });
  jqNoConflict(document).on('click.note-dropdown-menu', function (e) {
      jqNoConflict(e.target).closest('.note-dropdown-menu').parent().removeClass('open');
  });
  ...
  var ModalUI = /** @class */ (function () {
      function ModalUI($node, options) {
          ...
          this.$modal = $node;
          this.$backdrop = jqNoConflict('<div class="note-modal-backdrop" />');
      }
  ...
  var button = renderer.create('<button type="button" class="note-btn" role="button" tabindex="-1">', function ($node, options) {
      ...
      if (options && options.tooltip) {
          ...
          $node.data('_lite_tooltip', new TooltipUI($node, {
              ...
              jqNoConflict(e.currentTarget).data('_lite_tooltip').hide();
          });
      }
      if (options.contents) {
  ...
  var dropdown = renderer.create('<div class="note-dropdown-menu" role="list">', function ($node, options) {
      var markup = Array.isArray(options.items) ? options.items.map(function (item) {
          ...
          var $temp = jqNoConflict('<a class="note-dropdown-item" href="#" data-value="' + value + '" role="listitem" aria-label="' + value + '"></a>');
          $temp.html(content).data('item', item);
          return $temp;
      }) : options.items;
      $node.html(markup).attr({ 'aria-label': options.title });
      $node.on('click', '> .note-dropdown-item', function (e) {
          var $a = jqNoConflict(this);
  ...
  var dropdownCheck = renderer.create('<div class="note-dropdown-menu note-check" role="list">', function ($node, options) {
      var markup = Array.isArray(options.items) ? options.items.map(function (item) {
          var value = (typeof item === 'string') ? item : (item.value || '');
          var content = options.template ? options.template(item) : item;
          var $temp = jqNoConflict('<a class="note-dropdown-item" href="#" data-value="' + value + '" role="listitem" aria-label="' + item + '"></a>');
  ...
  var tableMoveHandler = function (event, col, row) {
      var PX_PER_EM = 18;
      var $picker = jqNoConflict(event.target.parentNode); // target is mousecatcher
      ...
      if (event.offsetX === undefined) {
          var posCatcher = jqNoConflict(event.target).offset();
          posOffset = {
  ...
  var palette = renderer.create('<div class="note-color-palette"/>', function ($node, options) {
      ...
      $node.find('.note-color-btn').each(function () {
          jqNoConflict(this).data('_lite_tooltip', new TooltipUI(jqNoConflict(this), {
              container: options.container
          }));
      });
  });
          ...
          children: [
              ...
              dropdown({
                  ...
                  click: function (event) {
                      var $button = jqNoConflict(event.target);
  ...
  function rect2bnd(rect) {
      var $document = jqNoConflict(document);
      return {
