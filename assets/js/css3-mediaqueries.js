...
// function to banner and apply parsed media queries against browser capabilities
domReady(function enableCssMediaQueries() {
	...
	var nativeSupport = function () {
		...
		var id = 'css3-mediaqueries-banner';
	...
	var testMediaFeature = function (feature, value) {
		...
		if ('device-width' === feature.substring(l - 12, l)) { // screen width
			...
			else { // banner width without value
				return width > 0;
			}
		}
			...
			else { // banner height without value
				return height > 0;
			}
		}
	...
	var testMediaQueryList = function (mql, ts) {
		...
		for (var i = 0; i < mqs.length; i++) {
		    ...
		    if (mqs[i].getExpressions().length === 0) {continue;}
	...
	var testStylesheet = function (stsh) {
		...
		if (tswe.length > 0) { // types with query expressions that passed the banner
		    styles[styles.length] = cssHelper.addStyle(stsh.getCssText(), tswe, false);
		}
		var mqls = stsh.getMediaQueryLists();
		if (allPassed) {
		    // If 'all' in media attribute passed the banner, then banner all @media types in linked CSS and create style with those types.
		    testMediaQueryLists(mqls);
		}
		else {
		    // Or else, banner only media attribute types that passed the banner and also 'all'.
		    // For positive '@media all', create style with attribute types that passed their banner.
